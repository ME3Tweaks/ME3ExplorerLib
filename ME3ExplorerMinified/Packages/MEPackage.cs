    ...
    public sealed class MEPackage : UnrealPackageFile, IMEPackage
    {
        ...
        public void save(string path)
        {
            ...
            try
            {
                ...
                {
                    throw new Exception($"Cannot save ME1 packages with compressed textures. Please make an issue on github: {DLL.BugReportURL}");
                }
            }
            finally
            {
        ...
        private void saveByReconstructing(string path, bool isSaveAs)
        {
            ...
            //DLL won't handle this as it doesn't have MessageBox
            //}
            //catch (Exception ex) when (!DLL.IsDebug)
            //{
        ...
        private void ReadLocalTLKs()
        {
            ...
            foreach (var tlkFileSet in tlkFileSets)
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    exportsToLoad.Add(getUExport(maleTlk)); //DLL: Up to user to determine which TLK they need to use.
                    break;
                                                                                         ...
                                                                                         || mip.storageType == StorageTypes.pccZlib)))
            {
                ...
                using (var tfc = File.OpenWrite(Path.ChangeExtension(FilePath, "tfc")))
                {
                    ...
                    foreach (ExportEntry texport in exports.Where(exp => exp.IsTexture()))
                    {
                        ...
                        foreach (Embe.Texture2DMipInfo mipInfo in mips)
                        {
