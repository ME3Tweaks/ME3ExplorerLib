    ...
    public static class ME3UnrealObjectInfo
    {
        ...
        public static string LoadEmbeddedJSONText(MEGame game)
        {
            try
            {
                ...
                if (stream != null)
                {
                    ...
                    {
                        var objectinfoEntry = archive.Entries.FirstOrDefault(x => x.FullName == $"{game}ObjectInfo.json");
                        if (objectinfoEntry != null)
                        {
                            using (StreamReader reader = new StreamReader(objectinfoEntry.Open()))
                            {
        ...
        public static void loadfromJSON()
        {
            if (!loaded)
            {
                try
                {
                    var blob = JsonConvert.DeserializeAnonymousType(LoadEmbeddedJSONText(MEGame.ME3), new { SequenceObjects, Classes, Structs, Enums });
            ...
            if (Structs.ContainsKey(structName))
            {
                try
                {
                    ...
                    while (info != null)
                    {
                        ...
                        {
                            IMEPackage importPCC = MEPackageHandler.OpenME3Package(filepath);
